    ...
    <nav class="section-container h-20 flex-between">
      ...
      <div class="hidden md:flex items-center gap-8 text-base font-medium text-text-body">
        <router-link v-for="link in navLinks" :key="link.to"
          :to="link.to" class="hover:text-brand transition-colors">
          {{ link.label }}
        </router-link>
      </div>
    ...
    <Transition name="side">
      <div v-if="mobileMenuOpen" class="mobile-side flex flex-col">
        ...
        <div class="flex items-center justify-end px-6 pt-6 pb-2">
          <button @click="closeMenu" class="w-8 h-8 flex items-center justify-center rounded-full bg-border-subtle/50 text-text-body hover:bg-border-subtle transition-colors">
        ...
        <nav class="px-4 py-8 space-y-2 flex-1 overflow-y-auto flex flex-col justify-center">
          <router-link
            ...
            :key="link.to"
            :to="link.to"
            class="mobile-nav-item justify-center"
        ...
        <div class="px-4 pb-8 pt-4 border-t border-border-subtle mt-auto">
          ...
          <p class="text-center text-xs text-text-body/60 mt-3 font-medium">
            Безкоштовно · Без реєстрації
          </p>
          <div class="flex items-center justify-center gap-4 mt-6">
            <router-link to="/terms" class="text-xs font-semibold text-text-body/50 hover:text-text-heading transition-colors" @click="closeMenu">
              Умови
            </router-link>
            <span class="text-text-body/30 text-xs">•</span>
            <router-link to="/privacy" class="text-xs font-semibold text-text-body/50 hover:text-text-heading transition-colors" @click="closeMenu">
              Конфіденційність
            </router-link>
          </div>
        </div>
...
.mobile-nav-item:active { background: var(--bg-main); }



/* ── Transitions ── */
.overlay-enter-active,
.overlay-leave-active { transition: opacity 0.28s ease; }
.overlay-enter-from,
.overlay-leave-to { opacity: 0; }

.side-enter-active { transition: transform 0.35s cubic-bezier(0.32, 0.72, 0, 1); }
.side-leave-active { transition: transform 0.28s cubic-bezier(0.4, 0, 1, 1); }
.side-enter-from,
.side-leave-to { transform: translateX(100%); }
</style>
